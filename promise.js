// PROMISE
// Promises are objects which are used to perform asynchronous operations. it's immutable
// Promises are used to solve the problems of callbacks like inversion of control and callback hell.
// difference bw call back and promise is callback is passing function to another function and promise is attaching function
...
let promise8 = new Promise((resolve, reject) => {
  ...
  if (dataReceivedSuccessfully) resolve(some_array);
  //passing an Array as the single argument
  else reject(some_object);
...
promiseone = new Promise((resolve, reject) => {
  if (isRequestComplete) resolve("data received from  one");
});
promisetwo = new Promise((resolve, reject) => {
  if (isRequestComplete) resolve("data received from two");
});
...
promise1 = new Promise((resolve, reject) => {
  if (requestComplete) resolve("data received from requestComplete 1");
});
promise2 = new Promise((resolve, reject) => {
  if (requestComplete) resolve("data received from requestComplete 2");
});
...
promise4 = new Promise((resolve, reject) => {
  if (isCompleted) resolve("i won from 1");
});
promise5 = new Promise((resolve, reject) => {
  if (isCompleted) resolve("i won from 2");
});
...
Promise.race([Promise.reject(1), promise4, promise5, promise6, promise7]).then(
  (message) => {
    console.log(message);
  }
);
